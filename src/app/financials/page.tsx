    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden">
      <div className="p-4 border-b border-gray-200">
        <div className="flex justify-between items-center">
          ...
          {/* Toggle Buttons for Revenue/GP/NI/Expenses */}
          <div className="flex rounded-lg border border-gray-300 overflow-hidden">
            <button
            ...
            <button
              ...
              style={{ backgroundColor: propertyChartMetric === 'expenses' ? BRAND_COLORS.warning : undefined }}
            >
              Expenses
            </button>
          </div>
        ...
        <div className="text-sm text-gray-600 mt-2">
          {propertyChartMetric === 'income' ? 
            ...
            propertyChartMetric === 'expenses' ?
            `Total Expenses (Operating + Other) by property for ${timePeriod} period` :
            `Financial breakdown by property for ${timePeriod} period`
          }
          {viewMode === 'by-property' && (
            <span className="ml-2 text-purple-600">• Property View Active</span>
          )}
          {propertyChartMetric === 'expenses' && (
            <span className="ml-2 text-orange-600">• Higher values indicate more spending</span>
          )}
        </div>
      </div>
              ...
              <RechartsPieChart>
                <defs>
                  {generatePropertyChartData().map((entry, index) => {
                    // Use different color scheme for expenses
                    const colorSet = propertyChartMetric === 'expenses' 
                      ? ['#F59E0B', '#EF4444', '#DC2626', '#B91C1C', '#991B1B', '#7F1D1D'] // Warm colors for expenses
                      : COLORS; // Original colors for revenue/profit
                    
                    return (
                      <radialGradient key={`gradient-${index}`} id={`gradient-${index}`} cx="30%" cy="30%">
                        <stop offset="0%" stopColor={colorSet[index % colorSet.length]} stopOpacity="1" />
                        <stop offset="70%" stopColor={colorSet[index % colorSet.length]} stopOpacity="0.8" />
                        <stop offset="100%" stopColor={colorSet[index % colorSet.length]} stopOpacity="0.6" />
                      </radialGradient>
                    );
                  })}
                </defs>
                  ...
                  content={({ active, payload }) => {
                    if (active && payload && payload.length > 0) {
                      ...
                      const metricName = propertyChartMetric === 'income' ? 'Revenue' :
                                       ...
                                       propertyChartMetric === 'ni' ? 'Net Income' :
                                       propertyChartMetric === 'expenses' ? 'Total Expenses' : 'Value';
                      ...
                      return (
                        <div style={{ 
                          ...
                          {/* Show additional context for expenses */}
                          {propertyChartMetric === 'expenses' && (
                            <>
                              ...
                              {data.totalExpenses > 0 && (
                                <div style={{ color: '#ef4444', fontSize: '11px', marginTop: '1px' }}>
                                  💸 Cost center analysis
                                </div>
                              )}
                            </>
                          )}
                          {/* Show profitability context for other metrics */}
                          {propertyChartMetric === 'ni' && (
                            <div style={{ 
                              color: data.value >= 0 ? '#059669' : '#dc2626', 
                              fontSize: '11px', 
                              marginTop: '2px' 
                            }}>
                              {data.value >= 0 ? '✅ Profitable' : '❌ Loss making'}
                            </div>
                          )}
                        </div>
                      );
            ...
            <div className="text-center">
              ...
              {propertyChartMetric === 'expenses' && (
                <p className="text-sm mt-1 text-orange-600">
                  💡 Expenses view shows spending patterns across properties
                </p>
              )}
            </div>
          </div>
